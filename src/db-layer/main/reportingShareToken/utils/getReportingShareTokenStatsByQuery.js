...
const getReportingShareTokenStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(
          ReportingShareToken.count({ where: queryWithSoftDelete }),
        );
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            ReportingShareToken.sum(statParts[1], {
          ...
          promises.push(
            ReportingShareToken.avg(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            ReportingShareToken.min(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            ReportingShareToken.max(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
    ...
    if (promises.length == 0) {
      return await ReportingShareToken.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingReportingShareTokenStatsByQuery",
      err,
    );
  }
};

module.exports = getReportingShareTokenStatsByQuery;
