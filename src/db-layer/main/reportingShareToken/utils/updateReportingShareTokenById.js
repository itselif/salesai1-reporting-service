...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("reportingShareToken");
  await elasticIndexer.indexData(data);
};

const updateReportingShareTokenById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await ReportingShareToken.findOne({
    ...
    const [rowsCount, [dbDoc]] = await ReportingShareToken.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateReportingShareTokenById;
