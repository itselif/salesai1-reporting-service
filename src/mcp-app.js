...
app.post("/mcp", async (req, res) => {
  ...
  if (sessionIdHeader && sessions[sessionIdHeader]) {
    ...
    const {
      ...
      reportRequestMcpRouter,
      reportFileMcpRouter,
      reportPolicyMcpRouter,
      reportingShareTokenMcpRouter,
      getSessionRouter,
    } = require("mcpLayer")(initialHeaders);

    const sessionRouter = getSessionRouter(initialHeaders, "store");

    reportRequestMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
    ...
    reportFileMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
    ...
    reportPolicyMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
    ...
    reportingShareTokenMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
